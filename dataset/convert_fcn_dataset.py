...
flags = tf.app.flags
flags.DEFINE_string('data_dir', 'VOC2012', 'Root directory to raw pet dataset.')
flags.DEFINE_string('output_dir', 'output-dataset', 'Path to directory to output TFRecords.')
...
def dict_to_tf_example(data, label):   
    ...
    img_label = cv2.imdecode(np.fromfile(label,dtype = np.uint8),-1)  # 读取png格式的图像
    ...
    filename=os.path.basename(data)                # basename返回最后的图片名称
...
def create_tf_record(output_filename, file_pars):
    ...
    for (data,label) in file_pars:                           # data和label分别为jpg格式和png格式的图片
        try:
